    ...
    def acceptNavigationRequest(self, url, _type, isMainFrame):
        try:
            ...
            if scheme not in ['https', 'file']:
                log_debug("Blocked non-https/non-file URL: " + url.toString())
                return False
...
class BrowserTab(QWidget):
    ...
    def __init__(self, main_window, url=None):
        ...
        initial_url = QUrl(url) if url else QUrl(DEFAULT_HOME_URL)
    ...
    def update_title(self, title):
        self.title_changed.emit(title[:15])
...
class MainWindow(QMainWindow):
    ...
    def _set_window_icon(self):
        ...
        if os.path.exists(icon_path):
            ...
            log_debug("Icon not found: " + icon_path)

    def _init_ui(self, initial_url):
        self.tabs = QTabWidget()
        self.tabs.tabBar().setStyleSheet("QTabBar::tab {text-decoration:underline; border: 0px; margin-left: 5px; margin-top:3px; margin-right:5px; padding-left:10px;}")
